...
Production: browser → edge Nginx (`ava-assistant.duckdns.org` vs `nexus-project.duckdns.org`) → `/` to the Next.js standalone server (`127.0.0.1:3000`), `/api/` and `/actuator/health` to Spring Boot (`127.0.0.1:8080`). Nginx disables buffering on `/api/` so SSE is not held. Nexus does not bind port 80.
