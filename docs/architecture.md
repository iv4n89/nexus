...
Production: browser → Nginx `:80` → `/` to the Next.js standalone server (`:3000`), `/api/` and `/actuator/health` to Spring Boot (`:8080`). Nginx disables buffering on `/api/` so SSE (logs, deploy output, activity) is not held.
