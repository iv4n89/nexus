...
public class VpsTerminalWebSocketHandler extends AbstractWebSocketHandler {
    private static final Logger log = LoggerFactory.getLogger(VpsTerminalWebSocketHandler.class);
    private static final String ATTR_SESSION_ID = "nexus.terminal.sessionId";

    private final TerminalSessionManager sessions;

    public VpsTerminalWebSocketHandler(TerminalSessionManager sessions) {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        TerminalSessionManager.TerminalSession terminal = sessions.open(username, clientIp);
        session.getAttributes().put(ATTR_SESSION_ID, terminal.id());
        sessions.onOutput(terminal.id(), chunk -> sendBinary(session, chunk));
        session.sendMessage(new TextMessage("{\"type\":\"ready\",\"sessionId\":\"" + terminal.id() + "\"}"));
    }
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) {
        log.warn("Terminal websocket error: {}", exception.toString());
        UUID sessionId = (UUID) session.getAttributes().get(ATTR_SESSION_ID);
        if (sessionId != null) {
